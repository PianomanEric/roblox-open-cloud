...
export class DataStore {
    ...
    async getAsync(key: string): Promise<[any, DataStoreKeyInfo]> {
        return this.universe.client
            .get(this.path + '/datastore/entries/entry', {
                params: {
            ...
            .then((response): [any, DataStoreKeyInfo] => {
                ...
                const keyInfo: DataStoreKeyInfo = new DataStoreKeyInfo(
                    createdTime,
                    updatedTime,
                    version,
                    attributes,
    ...
    async setAsync(
        key: string,
        value: any,
        setOptions: DataStoreSetOptions = new DataStoreSetOptions(),
        metadata: DataStoreKeyMetadata = new DataStoreKeyMetadata(),
    ): Promise<DataStoreKeyInfo> {
        return this.universe.client
            .post(this.path + '/datastore/entries/entry', JSON.stringify(value), {
                params: {
                    datastoreName: this.name,
                    entryKey: key,
                    scope: this.scope,
    ...
    async removeAsync(key: string): Promise<boolean> {
        return this.universe.client
            .delete(this.path + '/datastore/entries/entry', {
                params: {
                    datastoreName: this.name,
                    entryKey: key,
                    scope: this.scope,
    ...
    async incrementAsync(
        ...
        incrementBy?: number,
        metadata: DataStoreKeyMetadata = new DataStoreKeyMetadata(),
    ): Promise<[any, DataStoreKeyInfo]> {
        return this.universe.client
            .post(
                this.path + '/datastore/entries/entry/increment',
                {},
                {
                    params: {
                        datastoreName: this.name,
